...
def taskSquare(draw):
	...
	nn1 = nn.NeuralNetwork(2, 0.1, 32, 500)
	nn1.addLayer(FullyConnectedLayer(2, 32))
	nn1.addLayer(FullyConnectedLayer(32, 2))
...
def taskSemiCircle(draw):
	...
	nn1 = nn.NeuralNetwork(2, 0.01, 10, 300)
	nn1.addLayer(FullyConnectedLayer(2, 32))
	nn1.addLayer(FullyConnectedLayer(32, 16))
	nn1.addLayer(FullyConnectedLayer(16, 2))
	# raise NotImplementedError
	###############################################
	nn1.train(XTrain, YTrain, XVal, YVal, False, True)
	pred, acc  = nn1.validate(XTest, YTest)
...
def taskMnist():
	...
	nn1 = nn.NeuralNetwork(10, 0.01, 64, 200)
	nn1.addLayer(FullyConnectedLayer(784, 128))
	nn1.addLayer(FullyConnectedLayer(128, 64))
...
def taskCifar10():	
	...
	nn1 = nn.NeuralNetwork(10, 0.1, 8, 50)
	nn1.addLayer(ConvolutionLayer([3,32,32], [3,3], 8, 1))
	nn1.addLayer(FlattenLayer())
	nn1.addLayer(FullyConnectedLayer(8*30*30, 128))
